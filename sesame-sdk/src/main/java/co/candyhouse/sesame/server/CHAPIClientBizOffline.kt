...
internal class CHAPIClientBizOffline : ICHAPIClientBiz {
    ...
    override fun putKey(key: CHUserKey, onResponse: CHResult<Any>) {
        L.d(TAG, "Offline mode: saving key locally")
        val device = CHDevice(
            deviceUUID = key.deviceUUID,
            deviceModel = key.deviceModel,
            keyIndex = key.keyIndex,
            secretKey = key.secretKey,
            sesame2PublicKey = key.sesame2PublicKey
        )
    ...
    override fun getDevicesList(onResponse: CHResult<Array<CHUserKey>>) {
        L.d(TAG, "Offline mode: getting devices from local DB")
        CHDB.CHSS2Model.getAllDB { result ->
            result.fold(
                onSuccess = { devices ->
                    val userKeys = devices.map { device ->
                        CHUserKey(
                            deviceUUID = device.deviceUUID,
                            deviceModel = device.deviceModel,
                            keyIndex = device.keyIndex,
                            secretKey = device.secretKey,
                            sesame2PublicKey = device.sesame2PublicKey,
                            deviceName = null,
                            keyLevel = 0
                        )
                    }.toTypedArray()
                    onResponse(Result.success(co.candyhouse.sesame.utils.CHResultState.CHResultStateNetworks(userKeys)))
                },
                onFailure = { t ->
    ...
    override fun removeKey(keyId: String, onResponse: CHResult<Any>) {
        L.d(TAG, "Offline mode: removing key locally")
        CHDB.CHSS2Model.deleteByDeviceId(keyId) {
            onResponse(Result.success(co.candyhouse.sesame.utils.CHResultState.CHResultStateNetworks(Any())))
        }
    }
