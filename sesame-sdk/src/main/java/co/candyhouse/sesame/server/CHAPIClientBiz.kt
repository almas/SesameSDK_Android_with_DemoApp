...
object CHAPIClientBiz : ICHAPIClientBiz {
    ...
    fun initialize(
        ...
        OfflineConfig.initialize(context)
        OfflineConfig.setEnabled(false)
        OfflineConfig.setOfflineMode(false)
        ...
        initialized = true
        L.d(TAG, "CHAPIClientBiz initialized. Online Mode: ${!OfflineConfig.isOfflineMode()}")
    }
    ...
    fun initializeLocalServerMode(
        ...
        initialized = true
        L.d(TAG, "CHAPIClientBiz initialized for Local Server. Endpoint: ${OfflineConfig.getServerEndpoint()}")
    }
    ...
    fun initializeOfflineMode(context: Context) {
        appContext = context.applicationContext
        OfflineConfig.initialize(context)
        OfflineConfig.setOfflineMode(true)
    ...
    fun switchOffline(isOffline: Boolean) {
        OfflineConfig.setOfflineMode(isOffline)
        if (isOffline) {
            currentClient = offlineClient
