...
class SesameFirebaseMessagingService : FirebaseMessagingService() {
    ...
    override fun onMessageReceived(remoteMessage: RemoteMessage) {
        if (OfflineConfig.isEnabled() || OfflineConfig.isOfflineMode()) {
            L.d(tag, "Firebase messaging is disabled in ${if (OfflineConfig.isOfflineMode()) "offline" else "local server"} mode")
            return
        }
    ...
    override fun onNewToken(token: String) {
        if (OfflineConfig.isEnabled() || OfflineConfig.isOfflineMode()) {
            L.d(tag, "FCM token registration disabled in ${if (OfflineConfig.isOfflineMode()) "offline" else "local server"} mode")
            return
        }
        // Original Firebase handling would go here
    }
